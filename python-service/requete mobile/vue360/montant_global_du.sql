-- Montant global dû (Vue 360) — répartition capital / intérêts / pénalités / charges
-- Source : CSTB_AUTO_SETTLE_BLOCK (CHARGE + EXIGIBLE)
-- Chargée par encours_repartition_query.py (_load_query)
...
EXIGIBLE_1 AS (
    SELECT
        ...
        NVL(COALESCE(pr.CONTRACT_REF_NO, ped.CONTRACT_REF_NO, id.CONTRACT_REF_NO), 0) AS NO_PRET,
    ...
    FULL OUTER JOIN PENALITE_DUE ped
       ...
       AND NVL(ped.CONTRACT_REF_NO, 0) = NVL(pr.CONTRACT_REF_NO, 0)
    FULL OUTER JOIN INTERET_DUE id
        ON id.ACCOUNT_NO = COALESCE(pr.ACCOUNT_NO, ped.ACCOUNT_NO)
       AND NVL(id.CONTRACT_REF_NO, 0) = NVL(
            COALESCE(pr.CONTRACT_REF_NO, ped.CONTRACT_REF_NO), 0
        )
),
EXIGIBLE AS (
    SELECT
