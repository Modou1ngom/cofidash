...
{
    private const TYPES_WITH = "ENUM('CLIENT', 'PRODUCTION', 'PRODUCTION_VOLUME', 'ENCOURS_CREDIT', 'COLLECT', 'PRODUCTION_ENCOURS', 'ENCOURS_EPARGNE_SIMPLE', 'ENCOURS_COMPTE', 'ENCOURS_EPARGNE_PROJET', 'ENCOURS_COMPTE_COURANT', 'ENCOURS_EPARGNE_PEP_SIMPLE', 'DEPOT_GARANTIE', 'EPARGNE', 'VIREMENT', 'EPARGNE_SIMPLE', 'EPARGNE_PROJET', 'VOLUME_DAT', 'NEW_DEAL', 'COMPTES_OUVERTS')";

    private const TYPES_WITHOUT = "ENUM('CLIENT', 'PRODUCTION', 'PRODUCTION_VOLUME', 'ENCOURS_CREDIT', 'COLLECT', 'PRODUCTION_ENCOURS', 'ENCOURS_EPARGNE_SIMPLE', 'ENCOURS_COMPTE', 'ENCOURS_EPARGNE_PROJET', 'ENCOURS_COMPTE_COURANT', 'ENCOURS_EPARGNE_PEP_SIMPLE', 'DEPOT_GARANTIE', 'EPARGNE', 'VIREMENT', 'EPARGNE_SIMPLE', 'EPARGNE_PROJET', 'VOLUME_DAT', 'NEW_DEAL')";

    private const SQLITE_WITH = "'CLIENT', 'PRODUCTION', 'PRODUCTION_VOLUME', 'ENCOURS_CREDIT', 'COLLECT', 'PRODUCTION_ENCOURS', 'ENCOURS_EPARGNE_SIMPLE', 'ENCOURS_COMPTE', 'ENCOURS_EPARGNE_PROJET', 'ENCOURS_COMPTE_COURANT', 'ENCOURS_EPARGNE_PEP_SIMPLE', 'DEPOT_GARANTIE', 'EPARGNE', 'VIREMENT', 'EPARGNE_SIMPLE', 'EPARGNE_PROJET', 'VOLUME_DAT', 'NEW_DEAL', 'COMPTES_OUVERTS'";

    private const SQLITE_WITHOUT = "'CLIENT', 'PRODUCTION', 'PRODUCTION_VOLUME', 'ENCOURS_CREDIT', 'COLLECT', 'PRODUCTION_ENCOURS', 'ENCOURS_EPARGNE_SIMPLE', 'ENCOURS_COMPTE', 'ENCOURS_EPARGNE_PROJET', 'ENCOURS_COMPTE_COURANT', 'ENCOURS_EPARGNE_PEP_SIMPLE', 'DEPOT_GARANTIE', 'EPARGNE', 'VIREMENT', 'EPARGNE_SIMPLE', 'EPARGNE_PROJET', 'VOLUME_DAT', 'NEW_DEAL'";
    ...
    public function up(): void
    {
        ...
        if ($driver === 'sqlite') {
            $this->recreateSqliteTable(self::SQLITE_WITH, excludeType: null);
        } elseif ($driver === 'mysql' || $driver === 'mariadb') {
            DB::statement('ALTER TABLE objectives MODIFY COLUMN type '.self::TYPES_WITH.' NOT NULL');
        } elseif ($driver === 'pgsql') {
            DB::statement("ALTER TYPE objectives_type_enum ADD VALUE IF NOT EXISTS 'COMPTES_OUVERTS'");
        }
    }
    ...
    public function down(): void
    {
        ...
        if ($driver === 'sqlite') {
            $this->recreateSqliteTable(self::SQLITE_WITHOUT, excludeType: 'COMPTES_OUVERTS');
        } elseif ($driver === 'mysql' || $driver === 'mariadb') {
            DB::table('objectives')->where('type', 'COMPTES_OUVERTS')->delete();
            DB::statement('ALTER TABLE objectives MODIFY COLUMN type '.self::TYPES_WITHOUT.' NOT NULL');
        }
    }

    private function recreateSqliteTable(string $typesList, ?string $excludeType): void
    {
        ...
        $where = $excludeType ? " WHERE type != '{$excludeType}'" : '';
        DB::statement("
